...
CPU是计算机系统的心脏。CPU的工作是运行程序指令并且协调所有可用的硬件模块来提供完整的用户体验。尽管PPU和APU各自运行在独立的电路上，但是仍然需要按CPU节拍前进，并且执行CPU发出的命令。

在跳转到实现之前，我们需要简单地讨论一下CPU完成工作需要哪些资源。

CPU唯二可访问的资源是内存和寄存器。

从编程的角度来看，内存只是一个1-bit单元的连续数组。NES CPU使用16位进行内存寻址，即可以寻址65536个不同的内存单元。
...
RAM可以通过 **[0x0000 … 0x2000]**的地址空间访问。

访问 **[0x2000 … 0x4020]** 被重定向到其他可用的NES硬件模块: PPU, APU, GamePads等。

访问 **[0x4020 .. 0x6000]** 是特殊的，这里是不同的卡带所使用的特殊空间。该空间由映射器控制，即卡带上的特殊电路。
